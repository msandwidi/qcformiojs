  ...
  _createClass(Webform, [{
    ...
    value: function focusOnComponent(key) {
      ...
      if (key) {
        ...
        var listenerFunction = function listenerFunction(e) {
          e.stopPropagation();

          _this15.formReady.then(function () {
            if (_this15.refs.errorRef && _this15.refs.errorRef.length) {
              _this15.refs.errorRef[0].focus();
            }
          });

          _this15.removeEventListener(component.refs.input[0], 'blur', listenerFunction);
        };
    ...
    value: function showErrors(error, triggerEvent) {
      ...
      errors.forEach(function (err) {
        ...
        components.forEach(function (path) {
          var component = _this16.getComponent(path, _lodash.default.identity);
      ...
      errors.forEach(function (err) {
        if (err) {
          var createListItem = function createListItem(message) {
            ...
            var li = _this16.ce('li', params);

            _this16.setContent(li, message);

            if (err.component && err.component.key) {
              li.dataset.componentKey = err.component.key;
            }

            _this16.appendTo(li, ul);
          };
      ...
      if (triggerEvent && this.refs.errorRef && this.refs.errorRef.length) {
        var withKeys = Array.from(this.refs.errorRef).filter(function (ref) {
          return !!ref.dataset.componentKey;
        });
    ...
    value: function deleteSubmission() {
      ...
      return this.formio.deleteSubmission().then(function () {
        _this17.emit('submissionDeleted', _this17.submission);
    ...
    value: function submitForm() {
      ...
      return new _nativePromiseOnly.default(function (resolve, reject) {
        ...
        var submission = (0, _utils.fastCloneDeep)(_this18.submission || {}); // Add in metadata about client submitting the form

        submission.metadata = submission.metadata || {};

        _lodash.default.defaults(submission.metadata, {
          timezone: _lodash.default.get(_this18, '_submission.metadata.timezone', (0, _utils.currentTimezone)()),
          offset: parseInt(_lodash.default.get(_this18, '_submission.metadata.offset', (0, _moment.default)().utcOffset()), 10),
        ...
        _this18.hook('beforeSubmit', _objectSpread({}, submission, {
          ...
          if (!isDraft && !_this18.checkValidity(submission.data, true, submission.data)) {
            return reject();
          }

          _this18.everyComponent(function (comp) {
          ...
          _this18.hook('customValidation', _objectSpread({}, submission, {
            component: options.component
          }), function (err) {
            if (err) {
              ...
              _this18.customErrors = err;
              return reject();
            }

            _this18.loading = true; // Use the form action to submit the form if available.

            if (_this18._form && _this18._form.action) {
              var method = submission.data._id && _this18._form.action.includes(submission.data._id) ? 'PUT' : 'POST';
              return _Formio.default.makeStaticRequest(_this18._form.action, method, submission.data, _this18.formio ? _this18.formio.options : {}).then(function (result) {
                return resolve({
                  submission: result,
                  saved: true
                });
            ...
            var submitFormio = _this18.formio;

            if (_this18.nosubmit || !submitFormio) {
              return resolve({
    ...
    value: function executeSubmit(options) {
      ...
      return this.submitForm(options).then(function (_ref2) {
        var submission = _ref2.submission,
            saved = _ref2.saved;
        return _this19.onSubmit(submission, saved);
      }).catch(function (err) {
        return _nativePromiseOnly.default.reject(_this19.onSubmissionError(err));
      });
    }
    /**
     * Submits the form.
    ...
    value: function submit(before, options) {
      ...
      if (!before) {
        return this.beforeSubmit(options).then(function () {
          return _this20.executeSubmit(options);
        });
      } else {
    ...
    value: function submitUrl(URL, headers) {
      ...
      if (headers && headers.length > 0) {
        headers.map(function (e) {
          if (e.header !== '' && e.value !== '') {
            settings.headers[e.header] = _this21.interpolate(e.value, submission);
          }
        });
      }
      ...
      if (API_URL && settings) {
        try {
          _Formio.default.makeStaticRequest(API_URL, settings.method, submission, {
            ...
            _this21.emit('requestDone');

            _this21.setAlert('success', '<p> Success </p>');
          });
        } catch (e) {
    ...
    get: function get() {
      ...
      return this.formReady.then(function () {
        return _get(_getPrototypeOf(Webform.prototype), "ready", _this23).then(function () {
          return _this23.loadingSubmission ? _this23.submissionReady : true;
        });
      });
    }
    /**
