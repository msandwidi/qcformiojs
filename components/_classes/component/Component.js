  ...
  _createClass(Component, [{
    ...
    value: function createModal(element, attr) {
      ...
      this.addEventListener(dialog, 'close', function () {
        return _this8.removeChildFrom(dialog, document.body);
      });
    ...
    value: function fieldLogic(data, row) {
      ...
      var changed = logics.reduce(function (changed, logic) {
        var result = FormioUtils.checkTrigger(newComponent, logic.trigger, row, data, _this9.root ? _this9.root._form : {}, _this9);
        return (result ? _this9.applyActions(newComponent, logic.actions, result, row, data) : false) || changed;
    ...
    value: function applyActions(newComponent, actions, result, row, data) {
      ...
      return actions.reduce(function (changed, action) {
        switch (action.type) {
          case 'property':
            {
              FormioUtils.setActionProperty(newComponent, action, result, row, data, _this10);
              var property = action.property.value;

              if (!_lodash.default.isEqual(_lodash.default.get(_this10.component, property), _lodash.default.get(newComponent, property))) {
                changed = true;
              }
          ...
          case 'value':
            {
              ...
              var newValue = _this10.evaluate(action.value, {
          ...
          case 'mergeComponentSchema':
            {
              var schema = _this10.evaluate(action.schemaDefinition, {
                value: _lodash.default.clone(_this10.getValue()),
                data: data,
                row: row,
                component: newComponent,
                result: result
    ...
    value: function addMessages(messages) {
      ...
      if (this.refs.messageContainer) {
        this.setContent(this.refs.messageContainer, messages.map(function (message) {
          return _this11.renderTemplate('message', message);
        }).join(''));
      }
    }
  }, {
    ...
    value: function checkComponentValidity(data, dirty, row) {
      ...
      return async ? check.then(function (messages) {
        return _this14.setComponentValidity(messages, dirty);
